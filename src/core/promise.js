...
class Promise {
	...
	constructor(exector) {
		...
		this.result = null;
		/** 存储新旧promise切换所需的数据
		 * {onFulfilled(当前),onRejected(当前),resolve(下个promise),reject(下个promise)} */
		this.handlers = [];
...
/** 处理resolveValue的边界值,并将结果传递给新promise
...
/** 更新promise实例的state & value,并顺序执行then绑定的callback
 * @param {Promise} promise 要更新的promise实例
 * @param { FULFILLED / REJECTED} state 要更新为的状态
 * @param {any} value 要更新的value
 */
...
/** 执行then回调,在当期promise和下个promise间进行状态传递
...
function handleCallback(handler, state, result) {
	...
	try {
		// 如果
		if (state === FULFILLED) isFunction(onFulfilled) ? resolve(onFulfilled(result)) : resolve(result);
